...
#            print(alpha_hard)
            ...
            w_max_hard = (1-alpha_hard).unsqueeze(-2) * aX + alpha_hard.unsqueeze(-2) * bX

            X = ((w_max_hard - w_max).detach() + w_max) @ combine_max.T.unsqueeze(-3) + ((w_min_hard - w_min).detach() + w_min) @ combine_min.T.unsqueeze(-3)

            x = ((torch.minimum(a, b) - (alpha * a + (1-alpha) * b)).detach() + (alpha * a + (1-alpha) * b)) @ combine_min.T \
                + ((torch.maximum(a, b) - ((1-alpha) * a + alpha * b)).detach() + ((1-alpha) * a + alpha * b)) @ combine_max.T
